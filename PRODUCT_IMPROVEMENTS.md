...
- 所有连接同时启用：聚合模型目录，模型请求严格回到其来源连接。

编辑采用分步草稿流程，全部字段确认后才写入配置；稳定 UUID 使改名不再搬迁密钥，删除连接会一并删除 SecretStorage 中的 API Key。

### 3. 提供可见的连接状态 ✅

在状态栏、模型选择器或连接管理界面展示：

- 每个连接的名称与 Base URL 主机名。
- 最后连接结果与检测时间。
- 每个连接及连接池汇总的已发现模型数量。
- 刷新、测试和编辑连接的快速入口。
...
已完成 OpenAI/Anthropic 流式与非流式最小探测、保守四态判定、按配置指纹持久化及状态展示。工具、图片、推理与缓存能力仍待后续专门探测；诊断暂不改变生产聊天路由。
